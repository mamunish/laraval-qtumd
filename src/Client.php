...
class Client
{
    ...
    /**
     * Makes request to bitcoincash Core.
     *
     * @param string $method
     * @param mixed  $params
     *
    ...
    public function request($method, $params = [])
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\QtumdException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\QtumdException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\QtumdException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to bitcoincash Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\QtumdException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\QtumdException(
    ...
    /**
     * Converts amount from satoshi to bitcoincash.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from bitcoincash to satoshi.
     *
     * @param float $amount
     *
     * @return int
